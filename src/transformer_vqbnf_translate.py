...
from src.loss import Tacotron2Loss as TransformerLoss
...
class Transformer(TTSInterface, torch.nn.Module):
    ...
    def __init__(self, args=None):
        ...
        self.idim = args.idim
        self.odim = args.odim
        self.num_labels = args.num_labels + 1
        ...
        if args.eprenet_conv_layers != 0:
            ...
            encoder_input_layer = torch.nn.Embedding(
                num_embeddings=args.idim, embedding_dim=args.adim, padding_idx=padding_idx
            )
        ...
        self.feat_out_labels = torch.nn.Linear(args.adim, self.num_labels * args.reduction_factor)
    ...
    def forward(self, xs, ilens, ys, ys_labels, olens, stop_labels, *args, **kwargs):
        """Calculate forward propagation.
        ...
        # (B, Lmax//r, num_labels * r) -> (B, Lmax//r * r, num_labels)
        logits_labels = self.feat_out_labels(zs).view(zs.size(0), -1, self.num_labels)
        ...
        if self.reduction_factor > 1:
            ...
            ys = ys[:, :max_olen]
            stop_labels = stop_labels[:, :max_olen]
            stop_labels = torch.scatter(
                stop_labels, 1, (olens - 1).unsqueeze(1), 1.0
            )  # see #3388

        # calculate loss values
        l1_loss, l2_loss, bce_loss, ctc_loss = self.criterion(
            after_outs, before_outs, logits, logits_labels, ys, ys_labels, stop_labels, ilens, olens
        )
        ...
        if self.loss_type == "L1":
            loss = closs + l1_loss + bce_loss
        elif self.loss_type == "L2":
            loss = closs + l2_loss + bce_loss
        elif self.loss_type == "L1+L2":
            loss = closs + l1_loss + l2_loss + bce_loss
        else:
